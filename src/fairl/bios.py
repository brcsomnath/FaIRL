...
from dataloader.bios_loader import Bios, BatchData
...
def get_parser():
    ...
    parser.add_argument("--dataset_path",
                        default="../../../../purging-embeddings/data/bios",
                        type=str,
                        help="Dataset name.")
    parser.add_argument(
        "--MODEL",
    ...
    parser.add_argument("--num_target_class", default=28, type=int)
    parser.add_argument("--num_protected_class", default=2, type=int)
...
class FaIRL:
    ...
    def save_models(self, args, netG, netD, bert_model, step):
        torch.save(netG, os.path.join(self.save_dir, f'netG-{step}.pb'))
        torch.save(netD, os.path.join(self.save_dir, f'netD-{step}.pb'))
        torch.save(bert_model, os.path.join(self.save_dir, f'bert-{step}.pb'))

    def generate_debiased_embeddings(self, dataset_loader, model, netG):
        """
        ...
        for data, y, z in (dataset_loader):
            real_data = data.to(self.device).long()

            with torch.no_grad():
                output = netG(model(real_data)[1])
    ...
    def trainer(self, args):
        ...
        for step in range(self.dataset.iterations + 1):
            ...
            val_data = DataLoader(BatchData(test[0], test[1], test[2]),
                                  batch_size=args.batch_size,
                                  shuffle=False)
            old_val_data = DataLoader(BatchData(old_test[0], old_test[1],
                                                old_test[2]),
                                      batch_size=args.batch_size,
                                      shuffle=False)
            ...
            for i, epoch in enumerate(itr, 0):
                ...
                if step > 0:
                    ...
                    old_data = old_batch.to(self.device).long()

                    Z_old = previous_netG(previous_bert(old_data)[1]).detach()

                    total_labels = max(old_label) + 1
                    _, (_, _, z_old_losses, _) = mcr_loss.deltaR(
                        (Z_old), old_label, total_labels)

                for _, (batch, label, bias) in enumerate(train_data):
                    real_data = batch.to(self.device).long()
                    ...
                    bert_out = bert_model(real_data)[1]
                    Z = netG(bert_out)
                    Z_bar = netD(Z.detach())

                    disc_loss, comp = mcr_loss.deltaR((Z_bar), bias,
                    ...
                    task_loss, _ = mcr_loss.deltaR(Z, label, min((step + 1) * self.step_size, args.num_target_class))
                    bias_loss, _ = mcr_loss.deltaR((Z_bar), bias,
                                                   args.num_protected_class)
                    ...
                    old_bias_loss = 0.
                    if step > 0:
                        Z = netG(bert_model(old_data)[1])

                        _, (_, _, z_losses, _) = mcr_loss.deltaR(Z, old_label, total_labels)

                        R_zjzjold = 0.
                        for j in range(total_labels):
                            new_z = torch.cat((Z[old_label == j], Z_old[old_label == j]), 0)
                            R_zjzjold += mcr_loss.compute_discrimn_loss(new_z.T)
                        ...
                        Z_bar = netD(Z)
                        old_bias_loss, _ = mcr_loss.deltaR(Z_bar, old_bias, args.num_protected_class)
            ...
            train_dataset = self.generate_debiased_embeddings(
                train_data, bert_model, netG)
            test_dataset = self.generate_debiased_embeddings(
                val_data, bert_model, netG)
            old_test_dataset = self.generate_debiased_embeddings(
                old_val_data, bert_model, netG)
            print("Done!")

            data_train, task_label_train, bias_label_train = get_data(train_dataset)
            data_test, task_label_test, bias_label_test = get_data(test_dataset)
            old_data_test, old_task_label_test, old_bias_label_test = get_data(old_test_dataset)

            print("Sampling exemplars ...")

            # Exemplar Sampling
            labels = train[1]
            reserve_idx = self.sample_exemplars(args, train[1], data_train)
            
            reserve_x.extend([train[0][idx] for idx in reserve_idx])
            reserve_y.extend([train[1][idx] for idx in reserve_idx])
            reserve_g.extend([train[2][idx] for idx in reserve_idx])

            reserve = DataLoader(BatchData(reserve_x, reserve_y, reserve_g),
                                 batch_size=args.batch_size,
                                 shuffle=False)
            print("Done!")

            self.save_models(args, netG, netD, bert_model, step)
            ...
            if step > 0:
                ...
                old_train_data = DataLoader(BatchData(old_train[0],
                                                      old_train[1],
                                                      old_train[2]),
                                            batch_size=args.batch_size,
                                            shuffle=True)
                old_train_dataset = self.generate_debiased_embeddings(old_train_data, bert_model, netG)

                old_data_train, old_task_label_train, old_bias_label_train = get_data(old_train_dataset)
                ...
                old_tpr = TPR_rms(old_task_label_test, y_pred_task_old,
                                  old_bias_label_test)
                print(f"Majority baseline: {maj_baseline} Leakage: {old_leakage}")
...
if __name__ == '__main__':
    ...
    bios = Bios(args.dataset_path, step_size=args.step_size)

    trainer = FaIRL(bios, device=args.device)
